...
export default function Videos({ title, featured = false, randomize = true }) {
  const thumbnails = [
  ...
  const [displayedThumbnails] = useState(() =>
    randomize ? shuffle(thumbnails) : thumbnails
  );

  return (
    <div
      className={`ml-4 sm:ml-10 lg:ml-[75px] ${
        featured ? "relative z-10 mt-[-70px] sm:mt-[-130px]" : ""
      }`}
    >
