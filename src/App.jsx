...
function App() {
  ...
  return (
    <div className='flex-col w-full'>
        ...
        <ul className='flex gap-[2vw] justify-center'>

          {
            profiles.map((profile, index)=>{
              return(
                <li className='hover:text-white text-[#808080] '>
                  <a href="">
                    <div style={{backgroundImage: `url(${profile.url})`}} className='h-[10vw] w-[10vw] bg-cover rounded-[4px] hover:outline-[3px] hover:outline '></div>
                    <span className='text-center block my-[.6em] text-[1.3vw]  '>{profile.name}</span>
                  </a>
              </li>
              )
            })
      ...
      <span className=' px-[1.5em] py-[0.5em] text-[#808080]  hover:text-white border border-[#808080] hover:border-white cursor-pointer'>Gérer les profils</span>
      </div>
    </div>
  )
}
